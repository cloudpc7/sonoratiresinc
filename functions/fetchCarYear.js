...
export const fetchCarYear = functions.https.onRequest((req, res) => {
  return cors({ origin: true })(req, res, async () => {
    ...
    try {
      ...
      const url = `https://api.wheel-size.com/v2/years/?make=${make}&model=${model}&region=usdm&user_key=3ee807b5af97fd58bf363faef19cb7fd`;
      ...
      const carYearData = await response.json();

      res.status(200).json({ success: true, data: carYearData });
    } catch (error) {
